...
const Admin_Room_List = () => {
  ...
  console.log("object_11222", roomTypes[0]?.name);
  ...
  const fetchRooms = async () => {
    try {
      ...
      if (
        formattedCheckIn &&
        formattedCheckOut &&
        formattedCheckIn.getTime() === formattedCheckOut.getTime()
      ) {
        formattedCheckOut.setDate(formattedCheckOut.getDate() + 1);
      }

      // Convert to yyyy-mm-dd for query
      const checkInStr = formattedCheckIn
        ? formattedCheckIn.toISOString().split("T")[0]
        : "";
      const checkOutStr = formattedCheckOut
        ? formattedCheckOut.toISOString().split("T")[0]
        : "";
