...
const Admin_Invoice = () => {
  ...
  // Load bookings and guests for dropdown
  const loadDependencies = async () => {
    try {
      const bookingRes = await Admin_Get_Rooms_Booking_list("", "", "");
      const guestRes = await Admin_Get_Rooms_Guest_list(1, 50);
  ...
  return (
    <AdminLayout>
      <div className="container mt-4">
        ...
        <div className="d-flex justify-content-between mb-3">
          <h3>Invoice List</h3>
          <button className="primary-button btn-sm small-add-button" onClick={() => setShowInvoiceFormModal(true)}>+ Invoice</button>
        </div>
            ...
            <tbody>
              {invoices.map((inv) => (
                <tr key={inv._id}>
                  ...
                  <td>${inv.paidAmount}</td>
                  <td>${inv.balance}</td>
                  <td className={inv.status === "paid" ? "text-success" : "text-danger"}>{inv.status}</td>
                  <td>{inv.dueDate?.split("T")[0]}</td>
                  <td>{inv.createdAt?.split("T")[0]}</td>
                  <td><FaEye className="text-success" style={{ cursor: "pointer" }} onClick={() => openItemModal(inv)} /></td>
                  <td><FaEye className="text-success" style={{ cursor: "pointer" }} onClick={() => openInvoiceModal(inv._id)} /></td>
                  <td>
                    <button className="primary-button btn-sm small-add-button" onClick={() => openPaymentModal(inv)}>+ Payment</button>
                    <button className="secondary-button btn-sm mt-2 small-add-button" onClick={() => openExpenseModal(inv)}>+ Expense</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {/* Invoice Form Modal */}
        <Modal show={showInvoiceFormModal} onHide={() => setShowInvoiceFormModal(false)} size="lg">
          <Modal.Header closeButton>
          ...
          <Modal.Body>
            <Form>
              <Row>
                <Col md={6}>
                  <Form.Group>
                    ...
                    <Form.Select value={formData.booking} onChange={(e) => setFormData({ ...formData, booking: e.target.value })}>
                      <option>Select booking</option>
                      {bookings.map((b) => <option key={b._id} value={b._id}>{b.bookingNumber}</option>)}
                    </Form.Select>
                  </Form.Group>
                </Col>
                <Col md={6}>
                  <Form.Group>
                    ...
                    <Form.Select value={formData.guest} onChange={(e) => setFormData({ ...formData, guest: e.target.value })}>
                      <option>Select guest</option>
              ...
              <Table striped bordered hover responsive className="table-smaller">
                <thead>
                  <tr>
                    <th>Description</th><th>Qty</th><th>Unit Price</th><th>Tax</th>
                  </tr>
                </thead>
                <tbody>
                  {formData.items.map((item, idx) => (
                    <tr key={idx}>
                      <td><Form.Control value={item.description} onChange={(e) => updateItem(idx, "description", e.target.value)} /></td>
                      <td><Form.Control type="number" value={item.qty} onChange={(e) => updateItem(idx, "qty", e.target.value)} /></td>
                      <td><Form.Control type="number" value={item.unitPrice} onChange={(e) => updateItem(idx, "unitPrice", e.target.value)} /></td>
                      <td><Form.Control type="number" value={item.tax} onChange={(e) => updateItem(idx, "tax", e.target.value)} /></td>
                    </tr>
                  ))}
                </tbody>
              </Table>
              <button type="button" className="secondary-button btn-sm" onClick={addItem}>+ Add Item</button>
            </Form>
          </Modal.Body>
          <Modal.Footer>
            <button className="secondary-button btn-sm" onClick={() => setShowInvoiceFormModal(false)}>Cancel</button>
            <button className="primary-button btn-sm" onClick={handleSubmitInvoice}>Create Invoice</button>
          </Modal.Footer>
        </Modal>
        ...
        <Modal show={showItemModal} onHide={closeItemModal} size="lg" centered>
          <Modal.Header closeButton>
            <Modal.Title>Invoice Items — {selectedItem?.invoiceNumber}</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            {selectedItem?.items?.length > 0 ? (
          ...
          <Modal.Footer>
            <button className="secondary-button btn-sm" onClick={closeItemModal}>Close</button>
          </Modal.Footer>
        </Modal>
          ...
          <Modal.Body>
            {selectedInvoice && (
              <Form onSubmit={handleCreatePayment}>
                ...
                <Form.Group className="mb-3">
                  <Form.Label>Transaction ID</Form.Label>
                  <Form.Control type="text" name="transactionId" placeholder="Optional" />
                </Form.Group>

                <button type="submit" className="primary-button btn-sm">Submit Payment</button>
              </Form>
            )}
          </Modal.Body>
        </Modal>
        ...
        {/* Expense Modal */}
        <Modal show={showExpenseModal} onHide={closeExpenseModal} centered>
          <Modal.Header closeButton>
          ...
          <Modal.Body>
            {selectedInvoice && (
              <Form onSubmit={handleCreateExpense}>
                ...
                <Form.Group className="mb-3">
                  <Form.Label>Date Paid</Form.Label>
                  <Form.Control type="date" name="paidAt" defaultValue={new Date().toISOString().split("T")[0]} />
                </Form.Group>
                ...
                <button type="submit" className="primary-button btn-sm">Submit Expense</button>
              </Form>
            )}
          </Modal.Body>
        </Modal>
